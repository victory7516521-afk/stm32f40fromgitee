...
void Usart1_Init(int myBaudRate)
{
	...
	USART_InitStruct.USART_Mode		= USART_Mode_Tx|USART_Mode_Rx;	//双全工
...
void Usart3_Init(int myBaudRate)
{
	...
	//3、GPIO 初始化设置：要设置模式为复用功能。
	GPIO_InitStructure.GPIO_Pin 	= GPIO_Pin_11|GPIO_Pin_10; 		//引脚 -- 改
	GPIO_InitStructure.GPIO_Mode 	= GPIO_Mode_AF;		//复用功能 
	GPIO_InitStructure.GPIO_Speed 	= GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_OType 	= GPIO_OType_PP;
	GPIO_InitStructure.GPIO_PuPd 	= GPIO_PuPd_UP ;
	...
	//6、开启中断并且初始化 NVIC，使能中断（如果需要开启串口中断才需要这个步骤）。
	NVIC_InitStructure.NVIC_IRQChannel 					 = USART3_IRQn; //改
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;	//-- 看情况改
	NVIC_InitStructure.NVIC_IRQChannelSubPriority        = 1;	//-- 看情况改
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStructure);
