...
// 
// 参数说明：
// DMA_Streamx: DMA流选择，比如DMA2_Stream7
...
{ 
	...
	// 设置传输方向：从内存到外设（Memory To Peripheral）
	// 还有其他方向：外设到内存、内存到内存
	...
	// 外设地址是否自动增加？否（Disable）
	// 因为外设地址是固定的（比如串口的数据寄存器只有一个地址）
	DMA_InitStruct.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
	
	// 内存地址是否自动增加？是（Enable）
	// 因为要依次读取内存中的每个字节，地址需要递增
	DMA_InitStruct.DMA_MemoryInc = DMA_MemoryInc_Enable;
	
	// 外设数据宽度：字节（Byte），也就是8位
	DMA_InitStruct.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;
	
	// 内存数据宽度：字节（Byte），和外设保持一致
	DMA_InitStruct.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
	
	// DMA模式：普通模式（Normal）
	// 普通模式：传输完就停止，需要重新配置才能再次传输
	// 循环模式：传输完自动重新开始，适合连续传输（比如ADC采样）
	DMA_InitStruct.DMA_Mode = DMA_Mode_Normal;
	
	// DMA优先级：高（High）
	// 优先级分为：低、中、高、非常高
	// 当多个DMA同时申请总线时，优先级高的先执行
	DMA_InitStruct.DMA_Priority = DMA_Priority_High;
	
	// 内存突发模式：单次（Single）
	// 突发模式用于连续传输时提高效率，这里设为单次传输
