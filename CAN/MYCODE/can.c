...
//CAN初始化
//
//tsjw:重新同步跳跃时间单元.范围:CAN_SJW_1tq~ CAN_SJW_4tqtsjw，同步跳转宽度，用于补偿节点时钟误差，保证长期通信同步
...
u8 CAN1_Mode_Init(u8 tsjw,u8 tbs2,u8 tbs1,u16 brp,u8 mode)
{
	...
	CAN_InitStructure.CAN_TXFP=DISABLE;	//优先级由报文标识符(ID)决定 
    ...
    uint8_t CanFilterListRTR1 = 0x00;//远程请求标志位：0x00数据帧,0x01远程帧(遥控帧)
	//配置过滤器--屏蔽位模式
...
//	//Low占低16位  15~0位
//	CAN_FilterInitStructure.CAN_FilterIdLow=  0x0000; //不使用扩展帧，IDE=0(标准帧) RTR=0(数据帧)
//	
//	//掩码寄存器
...
//所有小于 int 的整数类型（如 uint8_t、uint16_t）在进行算术或位运算时，
//都会先提升为 int（或 unsigned int）,等于说CanFilterListSTID1是32位的
	...
	//掩码寄存器
	CAN_FilterInitStructure.CAN_FilterMaskIdHigh= 0xF000; //只接受7开头的报文
	CAN_FilterInitStructure.CAN_FilterMaskIdLow =0xFFFD;  //D(1101) -- 标准帧可过  数据帧与遥控可过



	CAN_FilterInitStructure.CAN_FilterFIFOAssignment=CAN_Filter_FIFO0;//过滤器0关联到FIFO0，把当前这个过滤器接收到的数据，放到接收 FIFO0 中。
...
#if CAN1_RX0_INT_ENABLE
	//CAN_IT_FMP0 ：中断条件是当FIFO里面有数据就触发中断
...
{		   		   
    ...
    CAN_Receive(CAN1, CAN_FIFO0, &Num);//读取数据	
    for(i=0;i<Num.DLC;i++)
    buf[i]=Num.Data[i];  
	return Num.DLC;	
}
